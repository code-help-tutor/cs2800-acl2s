...
(definec last-n-aux (l :tl skip :int) :tl
  (cond ((lendp l) nil)
        ((<= skip 0) (cons (head l) (last-n-aux (tail l) 0)))
        (t (last-n-aux (tail l) (1- skip)))))

(definec last-n (l :tl n :nat) :tl
  (last-n-aux l (- (len l) n))) 
...
(definec last-n-aux-pm (l :tl skip :int) :tl
  (match l
    (nil nil)
    ((f . r) (if (<= skip 0)
                 (cons f (last-n-aux r 0))
               (last-n-aux (tail l) (1- skip))))))

(definec last-n-pm (l :tl n :nat) :tl
  (last-n-aux-pm l (- (len l) n))) 
...
(definec rr (l :tl n :nat) :tl
  (match l
    ...
    (& (let* ((len-l (len l))
              (skip (mod n len-l)))
         (app2 (last-n l skip)
               (first-n l (- len-l skip)))))))

; Or define it as per property 3.
...
; Here is how we can define rl in terms of rr and vice versa.
(property (l :tl n :nat)
  :proofs? nil
  (^ (== (rl l n)
         (rr l (if (lendp l) 0 (- (len l) (mod n (len l))))))
     (== (rr l n)
         (rl l (if (lendp l) 0 (- (len l) (mod n (len l))))))))
