...
#|
 ...
 - Due on Friday, Nov. 7 by 11:00 PM.
...
; This is designed to count exactly the number of steps required.
; The key insight is to consider the case where b<=a first and
; then to use reduction!

(definec m1-b<=a (a b :nat) :nat
  :ic (<= b a)
  (if (evenp (- a b))
      (/ (- a b) 2)
    (+ 2 (/ (- (1+ a) b) 2))))
...
(property (x :tl y :int) 
  :hyps (^ (!= y (len x))
           (<= y (len x)))
  (< (m2 x (+ 1 y (len x))) (m2 x y)))

"Property 2-2"
(property (x :tl y :int) 
  :hyps (^ (!= y (len x))
           (! (<= y (len x))))
  (< (m2 (cons y x) y) (m2 x y)))
...
#|
 ...
 We provide the the property below for you since it is tricky.
 Here is the reasoning. In this recursive call of f7:

 (f7 (1- n) (f7 n (1- m)))

 we have an argument that involves f7, which we have not admitted. Now
 as per RAP, you can use that in a measure function, but to do it in a
 property before the function is admitted is beyond what I want you to
 worry about. So, this is a reasonable alternative, where you
 introduce a new argument, x, that we know nothing about (it can be
 anything) and your measure function has to work for it anyway.

|#

"Property 7-3"
(property (n m :nat x :all)
  :hyps (^ (! (zp n))
           (! (zp m)))
  (l< (m7 (1- n) x) (m7 n m)))

(definec-no-test f7 (n m :nat) :pos
