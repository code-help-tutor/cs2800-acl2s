...
= { DeMorgan, Double Negation }
 (a => b) ^ (c => a) <> (a ^ b ^ (a v c))
= { Shannon exp a, Const prop, Double Negation }
 [a ^ (b <> b)] v [!a ^ (!c <> F)]
= { b <> b = F, Const prop }
 [!a ^ !c]
= { Def !v }
...
; does not mean you have the simplest formula.
"Property A4"
(property (a b c d :bool)
  (== (xor (^ (=> a b) (=> c a)) (!v (!^ a b) (^ (! a) (! c))))
      (!v a c)))
...
(definec BfEval (p :BoolFm a :assignment) :bool
  (match p
    ...
    ((q '<> r) (xor (BfEval q a) (BfEval r a)))
    ((q '!v r) (! (v (BfEval q a) (BfEval r a))))
    ((q '!^ r) (! (^ (BfEval q a) (BfEval r a))))))

;; We need 16 check=s for contract coverage, since there are 8 cases
;; for BoolFm (bool, var, unary negation, and the 5 binary operations)
;; and 2 cases for assignment (consp and endp)
;; Here we go...

;; bool cases
(check= (BfEval t nil) t)
(check= (BfEval t '(a b)) t)
;; var cases
(check= (BfEval 'a nil) nil)
(check= (BfEval 'a '(b a c)) t)
;; unary negation cases
(check= (BfEval '(! a) nil) t)
(check= (BfEval '(! nil) '(b a)) t)
;; binary operator ^
(check= (BfEval '(a ^ (! b)) nil) nil)
(check= (BfEval '(a ^ (! b)) '(a)) t)
;; binary operator v
(check= (BfEval '(a v (! b)) nil) t)
(check= (BfEval '((! a) v (! b)) '(c)) t)
;; binary operator <>
(check= (BfEval '(a <> a) nil) nil)
(check= (BfEval '((a v b) <> ((! c) v a)) '(a b)) nil)
;; binary operator !v
(check= (BfEval '(a !v ((! b) <> c)) nil) nil)
(check= (BfEval '(a !v b) '(a)) nil)
;; binary operator !^
(check= (BfEval '(a !^ c) nil) t)
(check= (BfEval '(b !^ c) '(a b)) t)

;; Now, for expression coverage. One might think we have enough
;; check=s, but we haven't hit all cases for the ands and ors in
;; BfEval, particularly for the !^ evaluation.
;; We need to hit the case where both arguments evaluate to t
;; Here is a check= that will hit it:
(check= (BfEval '(a !^ b) '(a b)) nil)

;; And finally, 2 properties.
;; I decided to encode some some propositional logic properties from
;; the lecture notes
(property (p q :BoolFm a :assignment)
  (== (BfEval `(,p ^ (,p v ,q)) a)
      (BfEval p a)))

(property (p q r :BoolFm a :assignment)
  (=> (BfEval `((,p <> ,q) ^ (,q <> ,r)) a)
      (BfEval `(! (,p <> ,r)) a)))
...
(definec BoolFm->BoolFm1 (p :BoolFm) :BoolFm1
  (match p
    ...
    (('! q) `(! ,(BoolFm->BoolFm1 q)))
    ((q op r) `(,(BoolFm->BoolFm1 q) ,op ,(BoolFm->BoolFm1 r)))
    (& p)))

;; This function takes in a BoolFm, so we need 10 check=s for contract
;; coverage.  Lucky for us, we already wrote some! I'll just use the
;; cases I used for BfEval.
(check= (BoolFm->BoolFm1 t) t)
(check= (BoolFm->BoolFm1 'a) 'a)
(check= (BoolFm->BoolFm1 '(! a)) '(! a))
(check= (BoolFm->BoolFm1 '(a ^ (! b))) '(a ^ (! b)))
(check= (BoolFm->BoolFm1 '(a v (! b))) '(a v (! b)))
(check= (BoolFm->BoolFm1 '(a <> (b ^ c))) '(a <> (b ^ c)))
(check= (BoolFm->BoolFm1 '((a ^ b) !v c)) '((a ^ b) !v c))
(check= (BoolFm->BoolFm1 '(a <> a)) '(a <> a))
(check= (BoolFm->BoolFm1 '(a !v ((! b) v (! c)))) '(a !v ((! b) v (! c))))
;; This is the only case that should be transformed
(check= (BoolFm->BoolFm1 '(a !^ c)) '(! (a ^ c)))

;; This isn't required, but I'll add another test for good measure.
;; This ensures that our function is properly handling nested !^s
(check= (BoolFm->BoolFm1 '(a v (b !^ c))) '(a v (! (b ^ c))))

;; Now, some properties

;; BoolFm->BoolFm1 shouldn't do anything to a value that is already
;; a BoolFm1.
(property (p :BoolFm1)
  (== (BoolFm->BoolFm1 p)
      p))

;; If there are no nested !^'s except a top-level one, BoolFm->BoolFm1
;; just replaces the top-level !^.
(property (p q :BoolFm1)
  (== (BoolFm->BoolFm1 `(,p !^ ,q))
      `(! (,p ^ ,q))))

;; The rest of the solutions do not include complete tests. 
...
(definec BoolFm1->BoolFm2 (p :BoolFm1) :BoolFm2
  (match p
    ((q '<> r) (let ((nq (BoolFm1->BoolFm2 q))
                    (nr (BoolFm1->BoolFm2 r)))
                 `(((! ,nq) ^ ,nr) v (,nq ^ (! ,nr)))))
    (('! q) `(! ,(BoolFm1->BoolFm2 q)))
    ((q op r) `(,(BoolFm1->BoolFm2 q) ,op ,(BoolFm1->BoolFm2 r)))
    (& p)))
...
(definec BoolFm2->BoolFm3 (p :BoolFm2) :BoolFm3
  (match p
    ...
    (('! q) `(! ,(BoolFm2->BoolFm3 q)))
    ((q op r) `(,(BoolFm2->BoolFm3 q) ,op ,(BoolFm2->BoolFm3 r)))
    (& p)))
...
(definec BoolFm3->BoolFm4 (p :BoolFm3) :BoolFm4
  (match p
    ...
    (('! q) `(! ,(BoolFm3->BoolFm4 q)))
    ((q '!v r) `(,(BoolFm3->BoolFm4 q) !v ,(BoolFm3->BoolFm4 r)))
    (& p)))

"Property 4"
(property (p :BoolFm3 a :assignment)
  (== (BfEval (BoolFm3->BoolFm4 p) a)
      (BfEval p a)))
...
(definec BoolFm4->NorFm (p :BoolFm4) :NorFm
  (match p
    (('! q) `(nil !v ,(BoolFm4->NorFm q)))
    ((q '!v r) `(,(BoolFm4->NorFm q) !v ,(BoolFm4->NorFm r)))
    (& p)))

"Property 5"
(property (p :BoolFm4 a :assignment)
  (== (BfEval (BoolFm4->NorFm p) a)
      (BfEval p a)))
...
(definec BoolFm->5NorFm (p :BoolFm) :NorFm
  (let* ((p1 (BoolFm->BoolFm1 p))
         (p2 (BoolFm1->BoolFm2 p1))
         (p3 (BoolFm2->BoolFm3 p2))
         (p4 (BoolFm3->BoolFm4 p3))
         (p5 (BoolFm4->NorFm p4)))
    p5))
...
(definec BoolFm->NorFm (p :BoolFm) :NorFm
  (match p
    ((q '!^ r) (BoolFm->NorFm `(! (,q ^ ,r))))
    ((q '<> r) (let ((nq (BoolFm->NorFm q))
                     (nr (BoolFm->NorFm r)))
                 (BoolFm->NorFm `(((! ,nq) ^ ,nr) v (,nq ^ (! ,nr))))))
    ((q 'v r) (BoolFm->NorFm `(! (,q !v ,r))))
    ((q '^ r) (BoolFm->NorFm `((! ,q) !v (! ,r))))
    (('! q) `(nil !v ,(BoolFm->NorFm q)))
    ((q '!v r) `(,(BoolFm->NorFm q) !v ,(BoolFm->NorFm r)))
    (& p)))

; Note that let is used to avoid doing the same work multiple times.
...
(defconst *NorFM-large* '(nil !v nil))
(check (NorFMp *NorFM-large*))

(defconst *NorCPFm-small* t)
...
(defconst *NorFM-small* '(nil !v (a !v b)))
(check (NorFMp *NorFM-small*))

(defconst *NorCPFm-large* '((a !v b) !v (a !v b)))
